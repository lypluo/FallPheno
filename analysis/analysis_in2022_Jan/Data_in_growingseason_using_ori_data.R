...
#
site.names<-unique(df.merge$sitename)
#
df.new<-c()
...
#refer the code here: http://www.sthda.com/english/wiki/visualize-correlation-matrix-using-correlogramvo
png(filename = paste("./fig/using_ori_data/uni_corrlation_PhenovsGPP.png"),width = 768,height = 768)
...
#refer the code here: http://www.sthda.com/english/wiki/visualize-correlation-matrix-using-correlogramvo
png(filename = paste("./fig/using_ori_data/uni_corrlation_anomaly_PhenovsGPP.png"),width = 768,height = 768)
...
ggsave(p_ori,filename = paste0("./fig/using_ori_data/ori_lm_plot.png"))
...
ggsave(p_anomaly,filename = paste0("./fig/using_ori_data/anomaly_lm_plot.png"))

#########################################################################
# ##.only focus on the normal value
# ggscatter(df.new[df.new$A_GPP_NT_mean>c(-2.5)&df.new$A_GPP_NT_mean<2.5,],
#           x="A_GPP_NT_mean",y="A_eos",add = "reg.line")+
#   stat_smooth(method = "lm",formula = y ~ x)+
#   stat_cor(label.x = 1, label.y = 80) +
#   stat_regline_equation(label.x = 1, label.y = 72)
# lm_temp<-lm(data=df.new[df.new$A_GPP_NT_mean>c(-2.5)&df.new$A_GPP_NT_mean<2.5,],
#             A_eos ~ A_GPP_NT_mean)
# summary(lm_temp)
# 
# # ggplot(data=df.new[df.new$A_GPP_NT_mean>c(-2.5)&df.new$A_GPP_NT_mean<2.5,],aes(x=A_GPP_NT_mean,y=A_eos))+
# #   geom_point()+
# #   geom_smooth()
# 
# ggscatter(df.new[df.new$A_GPP_NT_cum>c(-600)&df.new$A_GPP_NT_cum<600,],
#           x="A_GPP_NT_cum",y="A_eos",add = "reg.line")+
#   stat_smooth(method = "lm",formula = y ~ x)+
#   stat_cor(label.x = 1, label.y = 80) +
#   stat_regline_equation(label.x = 1, label.y = 72)
# lm_temp<-lm(data = df.new[df.new$A_GPP_NT_cum>c(-600)&df.new$A_GPP_NT_cum<600,],
#             A_eos ~ A_GPP_NT_cum)
# summary(lm_temp)
# # ggplot(data=df.new,aes(x=A_GPP_NT_cum,y=A_eos))+
# #   geom_point()+
# #   geom_smooth()
