...
#and conduct the correlation analysis and robust regression
#####################################
library(dplyr)
#--------------------
#1.load the data
...
#mean GPP/NEP vs eos
lm_rob<-lmrob(eos~GPP_NT_mean,data=df.new)
summary(lm_rob)
...
p_eos_vs_GPP_NT<-ggscatter(df.new,
  x="GPP_NT_mean",y="eos",add = "reg.line")+
    stat_smooth(method = "lm",formula = y ~ x,col="black",fill="grey10")+
    stat_smooth(method = robustbase::lmrob,formula = y ~ x,
                col="red",fill=adjustcolor("tomato",0.1))+
...
#cum GPP/NEP vs eos
lm_rob<-lmrob(eos~GPP_NT_cum,data=df.new)
summary(lm_rob)
cbind(coef(lm_rob),confint(lm_rob, level = 0.95))
lm_reg<-lm(eos~GPP_NT_cum,data=df.new)
summary(lm_reg)
...
ggsave(p_ori,filename = paste0("./fig/using_filtered_data_robReg/ori_lm_plot.png"))
#(2) Anomaly mean and cum GPP/NEP vs eos
#Anomaly mean GPP/NEP vs eos
lm_rob<-lmrob(A_eos~A_GPP_NT_mean,data=df.new)
summary(lm_rob)
cbind(coef(lm_rob),confint(lm_rob, level = 0.95))
...
lm_rob<-lmrob(A_eos~A_GPP_NT_cum,data=df.new)
summary(lm_rob)
cbind(coef(lm_rob),confint(lm_rob, level = 0.95))
...
ggsave(p_anomaly,filename = paste0("./fig/using_filtered_data_robReg/anomaly_lm_plot.png"))
