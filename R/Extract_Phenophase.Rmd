...
library(lubridate)
#load the data
load(file = paste0("../data/df_GPP_Meteo_andVIs.rda"))
...
for(i in 1:length(DBF.sites)){
  ...
  ggplot(data=df.proc,aes(x=DoY,y=EVI,col=as.factor(Year)))+
    geom_point()
  ggplot(data=df.proc[df.proc$Year==2000,],aes(x=DoY,y=EVI,col=as.factor(Year)))+
    geom_point()
...
```{r echo=FALSE,message=FALSE,warning=FALSE}
source("../R/pheno_extraction_fun.R")
#DBF
#CA-Gro:2003-2014
#DE-Hai:2000-2012
#DE-Lnf:2002:2012
...
#
save(Phenos_final,file = paste0("../data/df_VIs_Phenos.rda"))
```
