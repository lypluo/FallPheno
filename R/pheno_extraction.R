...
SplinePheno_extraction<-function(data_ts,site.name,VI_name,do_norm,year){
  ...
  proc.VIs<-proc.ts %>%
    select(sitename,PFT,TIMESTAMP,Year,Month,NDVI,EVI,EVI_2,NIRv) %>%
    mutate(doy=yday(TIMESTAMP))
  
  #-----------
  #prepare to extract the phenolgoy
  #-----------
  real_time<-proc.VIs$TIMESTAMP
  ...
  #------------------------------------
  Pheno_metrics<-data.frame(matrix(ncol=1,nrow=20))
  for(a in 1:1){
    ...
    mn<-min(ts,na.rm=T)
    ampl<-max_line-mn
    trs_critrion<-mn+0.5*ampl
    abline(h=trs_critrion,col='blue',lty=2)
    #
    trs_sos50<-30+which.min(as.numeric(abs(trs_critrion - ts_sm[30:pop])))-1
    trs_eos50<-pop+which.min(as.numeric(abs(trs_critrion - ts_sm[pop:length(ts_sm)])))-1
    
    #important to set trs_eos1 and  trs_sos2 to NA if cannot find them##
    ifelse(exists('trs_eos1')==FALSE,trs_eos1<-NA,trs_eos1<-trs_eos1)
    ifelse(exists('trs_sos2')==FALSE,trs_sos2<-NA,trs_sos2<-trs_sos2)
    abline(v=c(trs_sos50,trs_eos50),col='blue')
    
    #add four more threshold-based PTDs
    trs_sos25_value<-mn+0.25*ampl
    trs_sos75_value<-mn+0.75*ampl
    trs_eos25_value<-mn+0.25*ampl
    trs_eos75_value<-mn+0.75*ampl
    trs_sos25<-15+which.min(as.numeric(abs(trs_sos25_value - ts_sm[15:pop])))-1
    trs_sos75<-15+which.min(as.numeric(abs(trs_sos75_value - ts_sm[15:pop])))-1
    trs_eos75<-pop+which.min(as.numeric(abs(trs_eos75_value - ts_sm[pop:length(ts_sm)])))-1
    trs_eos25<-pop+which.min(as.numeric(abs(trs_eos25_value - ts_sm[pop:length(ts_sm)])))-1
    ...
    Pheno_metrics[,a]<- c(UD,prrD,SD,pop,DD,psrD,RD,
                          trs_sos25,trs_sos50,trs_sos75,trs_eos75,trs_eos50,trs_eos25,
                          max_line,baseline,ampl,prr,psr,Gslope,Dslope)
  }
  Pheno_metrics<-as.data.frame(t(Pheno_metrics))
  names(Pheno_metrics)<-c("UD","prrD","SD","pop","DD","psrD","RD",
                              'trs_sos25','trs_sos50','trs_sos75','trs_eos75','trs_eos25','trs_eos25',
                              "maxline","baseline",'ampl',"prr","psr","Gslope","Dslope")
  ...
  metrics_trs<-c(trs_sos25,trs_sos50,trs_sos75,trs_eos75,trs_eos50,trs_eos25)
  ...
  names(metrics_trs)<-c('trs_sos25','trs_sos50','trs_sos75','trs_eos75','trs_eos50','trs_eos25')
  allparas<-c("UD"=UD,"prrD"=prrD,"SD"=SD,"pop"=pop,"DD"=DD,"psrD"=psrD,"RD"=RD,
              ...
              "trs_eos25"=trs_eos25,"trs_eos50"=trs_eos50,"trs_eos75"=trs_eos75,
              "maxline"=max_line,"baseline"=baseline,"ampl"=ampl,
...
# do_norm<-FALSE
# year<-2011
SplinePheno_metrics_plot<-function(Pheno_result,data_name,VI_name){
  ...
  par(mfrow=c(2,1))
  plot(ts,xlab=paste0(data_name,'_Date'),ylab=paste0(VI_name),
      main=paste0(data_name,' ',VI_name,' variation(spline)'),type='p',xaxt="n")
  ...
  trs_critrion<-plot_metrics$plot_paras$trs_critrion
  
  par(mfrow=c(2,1))
  ##plot1
  plot(ts,xlab=paste0(data_name,'_Spline_Date'),ylab=paste0('',VI_name),main=paste('phenophase-estimate'),xaxt='n',type = 'p')
  points(ts_sm,col='red')
  axis(1,at=c(0,30,61,91,122,153,181,212,242,273,303,334,365),labels=c("Sep",
                                                                       "Oct","Nov","Dec","Jan","Feb","Mar","Apr","May","Jun","Jul","Aug",'Sep'))
  #colors <- palette()[2:5]
  ...
  ##plot2
  plot(ts,xlab=paste0(data_name,'_Spline_Date'),ylab=paste0('',VI_name),
...
#plotting:
# Pheno_results<-SplinePheno_extraction(df,"DE-Hai","EVI",FALSE,2011)
# SplinePheno_metrics_plot(Pheno_results,"DE-Hai","EVI")
