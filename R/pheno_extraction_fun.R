...
SplinePheno_extraction<-function(data_ts,site.name,VI_name,do_norm,year){
  ...
  for(a in 1:1){
    ...
    if(min(as.numeric(abs(trs_critrion - ts_sm[30:pop])))>c(ampl*0.05)){
      trs_sos50<-NA
    }
    if(min(as.numeric(abs(trs_critrion - ts_sm[pop:length(ts_sm)])))>c(ampl*0.05)){
      trs_eos50<-NA
    }
    ...
    trs_25_value<-mn+0.25*ampl
    trs_75_value<-mn+0.75*ampl
    trs_sos25<-15+which.min(as.numeric(abs(trs_25_value - ts_sm[15:pop])))-1
    trs_sos75<-15+which.min(as.numeric(abs(trs_75_value - ts_sm[15:pop])))-1
    trs_eos75<-pop+which.min(as.numeric(abs(trs_75_value - ts_sm[pop:length(ts_sm)])))-1
    trs_eos25<-pop+which.min(as.numeric(abs(trs_25_value - ts_sm[pop:length(ts_sm)])))-1
    #set a crition to filter the pseduo trs25,trs75
    #if the minimum residual between trs_critrion and trs_sm bigger than 
    #0.1*ampl, then set the trs50<-NA
    abline(h=c(trs_25_value,trs_75_value),col='blue',lty=2)
    if(min(as.numeric(abs(trs_25_value - ts_sm[15:pop])))>c(ampl*0.05)){
      trs_sos25<-NA
    }
    if(min(as.numeric(abs(trs_75_value - ts_sm[15:pop])))>c(ampl*0.05)){
      trs_sos75<-NA
    }
    if(min(as.numeric(abs(trs_75_value - ts_sm[pop:length(ts_sm)])))>c(ampl*0.1)){
      trs_eos75<-NA
    }
    if(min(as.numeric(abs(trs_25_value - ts_sm[pop:length(ts_sm)])))>c(ampl*0.1)){
      trs_eos25<-NA
    }
...
# do_norm<-FALSE
# year<-2000
# Pheno_results<-SplinePheno_extraction(data_ts,site.name,VI_name,FALSE,2000)
...
#plotting:
# SplinePheno_metrics_plot(Pheno_results,"DE-Hai","EVI",2000)
